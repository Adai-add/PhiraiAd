...
impl Page for LibraryPage {
    ...
    fn touch(&mut self, touch: &Touch, s: &mut SharedState) -> Result<bool> {
        ...
        {
            if self.rks.touch(touch, t)? {
                if self.rks.editing && self.tabs.selected().ty != ChartListType::Local {
                    self.tabs.goto(s.rt, 0);
                    self.sync_local(s);
                }
                self.sync_rks_views();
                return Ok(true);
        ...
        // Leave the app's back button reachable while locking the category tabs.
        if self.rks.editing {
            return Ok(false);
        }
    ...
    fn update(&mut self, s: &mut SharedState) -> Result<()> {
        ...
        self.tabs.selected_mut().view.can_refresh = !self.rks.editing && (!is_local || self.current_fav_index.is_some());
