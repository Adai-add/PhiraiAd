...
impl ChartsView {
    pub fn rks_entry_available(&self) -> bool {
        self.multi_select.is_none() && !self.chart_menu.showing() && self.transit.is_none()
    }
                ...
                if let Some(chart) = &item.chart {
                    if self.rks_editing {
                        if let Some(path) = &chart.local_path {
                            if item.rks_check.touch(touch, t) {
                                item.btn.inner.cancel();
                                item.long_touch.reset();
                                button_hit();
                                self.rks_action = Some(crate::page::rks::RksAction::Toggle(path.clone()));
                                return Ok(true);
                            }
                            if item.rks_check.inner.touching() || item.rks_check.inner.contains(touch.position) {
                                item.btn.inner.cancel();
                                continue;
                            }
                        }
                    ...
                    if item.btn.touch(touch, t) {
                        if self.rks_editing {
                            item.long_touch.reset();
                            if let Some(path) = &chart.local_path {
                                self.rks_action = Some(crate::page::rks::RksAction::Edit(path.clone()));
                            }
                            return Ok(true);
                        }
                        item.long_touch.reset();
                            ...
                            item.btn.render_shadow(ui, r, t, |ui, path| {
                                ...
                                if let Some(chart) = &mut item.chart {
                                    ...
                                    let rks = self
                                        .rks_values
                                        .as_ref()
                                        .and_then(|values| chart.local_path.as_ref().and_then(|p| values.get(p)));
                                    let prefix = crate::custom_rks::level_prefix(&info.level);
                                    let number = rks
                                        .and_then(|r| r.difficulty)
                                        .map(crate::custom_rks::format_difficulty)
                                        .unwrap_or_else(|| {
                                            crate::custom_rks::difficulty_value(info.difficulty as f64)
                                                .map(crate::custom_rks::format_difficulty)
                                                .unwrap_or_else(|| "—".into())
                                        });
                                    let level = if rks.is_some() {
                                        format!("{prefix}{number}")
                                    } else if info.level.contains("Lv.") {
                                        info.level.clone()
                                    } else {
                                        format!("{} Lv.{}", info.level, info.difficulty as i32)
                                    };
                                    let mut size = 0.52 * r.w / cw;
                                    let natural = ui.text(&level).size(size).measure().w;
                                    if natural > r.w * 2. / 3. {
                                        size *= (r.w * 2. / 3.) / natural;
                                    }
                                    let x = r.right() - 0.016;
                                    let y = r.y + 0.016;
                                    let ms = ui.text(&level).pos(x, y).anchor(1., 0.).size(size).measure();
                                    ui.fill_path(
                                    ...
                                    if rks.is_some_and(|v| v.difficulty_changed || v.difficulty_ai) {
                                        let nw = ui.text(&number).size(size).measure().w;
                                        ui.text(&prefix).pos(x - nw, y).anchor(1., 0.).size(size).color(c).draw();
                                        ui.text(&number)
                                            .pos(x, y)
                                            .anchor(1., 0.)
                                            .size(size)
                                            .color(Color {
                                                a: c.a,
                                                ..Color::from_rgba(70, 230, 100, 255)
                                            })
                                            .draw();
                                    } else {
                                        ui.text(&level).pos(x, y).anchor(1., 0.).size(size).color(c).draw();
                                    }
                                    if let Some(rks) = rks {
                                        let accuracy = rks.accuracy.map(|a| format!("{a:.2}%")).unwrap_or_else(|| "—%".into());
                                        let accuracy_color = if rks.accuracy_custom {
                                            Color {
                                                a: c.a,
                                                ..Color::from_rgba(70, 230, 100, 255)
                                            }
                                        ...
                                        let mut text = ui
                                            .text(accuracy)
                                            .pos(x, ms.bottom() + 0.017)
                                            .anchor(1., 0.)
                                            .size(size * 0.78)
                                            .color(accuracy_color);
                                        let ar = text.measure();
                                        text.ui.fill_path(
                                            &ar.feather(0.006).rounded(0.007),
                                            Color {
                                                a: c.a * 0.7,
                                                ..text.ui.background()
                                            },
                                        ...
                                        text.draw();
                                        if rks.included || self.rks_editing {
                                            let check = Rect::new(r.x + 0.009, r.y + 0.009, 0.05, 0.05);
                                            // Expand the hit square by 30% about its centre; keep the visual size.
                                            item.rks_check.inner.set(ui, check.feather(check.w * 0.15));
                                            ui.fill_path(
                                                &check.rounded(0.007),
                                                if rks.included {
                                                    Color {
                                                        a: c.a,
                                                        ..Color::from_rgba(40, 175, 65, 255)
                                                    }
                                                } else {
                                                    Color {
                                                        a: c.a * 0.7,
                                            ...
                                            if rks.included {
                                                let mut tick = lyon::path::Path::builder();
                                                tick.begin(lyon::math::point(check.x + 0.01, check.y + 0.026));
                                                tick.line_to(lyon::math::point(check.x + 0.021, check.y + 0.036));
                                                tick.line_to(lyon::math::point(check.x + 0.041, check.y + 0.014));
                                                tick.end(false);
                                                ui.stroke_path(&tick.build(), 0.006, c);
                                            }
                                        } else {
                                            item.rks_check.invalidate();
                                        }
