...
use prpr::{
    ...
    fs::FileSystem,
    info::ChartInfo,
    scene::{BasicPlayer, GameMode, LoadingScene, NextScene, ReportFn, SaveFn, Scene, UpdateFn, UploadFn},
    time::TimeManager,
    ui::LoadingParams,
};
...
impl UnlockScene {
    ...
    pub async fn new(
        ...
        let loading_scene = Box::new(LoadingScene::new(mode, info, config, fs, player, upload_fn, update_fn, save_fn, report_fn, preloaded).await?);
