...
impl SongScene {
    pub fn new(mut chart: ChartItem, local_path: Option<String>, icons: Arc<Icons>, rank_icons: [SafeTexture; 8], mods: Mods) -> Self {
        ...
        if let Some(fallback) = local_path
            .as_ref()
            .and_then(|path| get_data().local_records.get(path))
            .and_then(|r| r.as_ref())
        {
            if let Some(record) = &mut record {
                record.update(fallback);
            } else {
                record = Some(fallback.clone());
            }
        }
        if let Some(local) = chart.info.id.and_then(|id| get_data().replica_local_records.get(&id)) {
            if let Some(record) = &mut record {
                record.update(local);
            } else {
                record = Some(local.clone());
            }
        }
    ...
    fn update_record(&mut self, new_rec: SimpleRecord) -> Result<()> {
            ...
            .and_then(|r| r.as_ref())
        {
            if let Some(record) = &mut self.record {
                ...
                self.record = Some(fallback.clone());
            }
        }
        if let Some(local) = self.info.id.and_then(|id| get_data().replica_local_records.get(&id)) {
            if let Some(record) = &mut self.record {
                record.update(local);
            } else {
                self.record = Some(local.clone());
            }
        }
        Ok(())
    }
    ...
    fn update_menu(&mut self) {
        ...
        if let Some(local_path) = &self.local_path {
            ...
            self.menu_options.push("chart-play-settings");
            if get_data()
                .charts
                .iter()
    ...
    pub fn global_launch(
        ...
        let save_fn: Option<SaveFn> = Some(Box::new({
            ...
            move |new_rec| -> Result<()> {
                if let Some(id) = id {
                    ...
                    if let Some(record) = data.replica_local_records.get_mut(&id) {
                        record.update(&new_rec);
                    } else {
                        data.replica_local_records.insert(id, new_rec);
                    }
                    save_data()?;
                    return Ok(());
                }
        ...
        let report_fn = Some(crate::play_report_export::report_fn());
        ...
        Ok(Some(Box::pin(async move {
            ...
            let replica_blocked = config.online_replica_active(info.id.is_some(), info.replica_play.blocks_score_upload());
            let upload_fn: Option<UploadFn> = if replica_blocked {
                None
            } else {
            ...
            if is_unlock {
                #[cfg(not(feature = "video"))]
                {
                    ...
                    LoadingScene::new(mode, info, config, fs, player, upload_fn, update_fn, save_fn, report_fn, Some(preload))
                        .await
                ...
                #[cfg(feature = "video")]
                {
                    ...
                    UnlockScene::new(mode, info, config, fs, player, upload_fn, update_fn, save_fn, report_fn, Some(preload))
                        .await
                        .map(|it| NextScene::Overlay(Box::new(it)))
                }
            } else {
    ...
    fn side_mods(&mut self, ui: &mut Ui, rt: f32) {
        ...
        self.mod_scroll.render(ui, |ui| {
            ...
            drop(item);
            let height = self.note_conversion_mods.render(ui, rt, width, self.chart_play_settings.note_conversion);
            dy!(height);
            (width, h + 0.2)
        });
    }
...
impl Scene for SongScene {
    fn on_result(&mut self, tm: &mut TimeManager, res: Box<dyn Any>) -> Result<()> {
        ...
        let res = match res.downcast::<prpr::chart_play::ChartPlaySettings>() {
            Ok(settings) => {
                if let Some(path) = self.local_path.as_deref() {
                    match crate::chart_play_settings::save(path, &settings) {
                        Ok(()) => {
                            show_message(tl!("edit-saved")).ok();
                        }
                        Err(error) => {
                            show_error(error.context("保存倒打设置失败；原文件未被截断"));
                        }
                    }
                }
                return Ok(());
    ...
    fn touch(&mut self, tm: &mut TimeManager, touch: &Touch) -> Result<bool> {
                ...
                match self.side_content {
                    ...
                    SideContent::Mods => {
                        ...
                        if let Some(mode) = self.note_conversion_mods.touch(touch, rt, self.chart_play_settings.note_conversion) {
                            ...
                            if let Some(path) = self.local_path.as_deref() {
                                match crate::chart_play_settings::save(path, &settings) {
                                    Ok(()) => self.chart_play_settings = settings,
                                    Err(error) => show_error(error.context("保存按键转换设置失败")),
                                }
                            }
                            return Ok(true);
                        }
        ...
        if let Some(path) = &self.local_path {
            ...
            if self.mod_btn.touch(touch) {
                self.chart_play_settings = crate::chart_play_settings::load(path)?;
                button_hit();
    ...
    fn update(&mut self, tm: &mut TimeManager) -> Result<()> {
        ...
        if self.menu.changed() {
            ...
            match option {
                ...
                "chart-play-settings" => {
                    self.launch(GameMode::EditChartPlay, false)?;
                }
                "unlock" => {
