...
impl<'a> Ui<'a> {
    ...
    /// Practice slider: capture the initiating finger until release/cancellation, even across empty frames.
    pub fn practice_speed_slider(
        &mut self,
        input_id: &str,
        ...
        STATE.with(|state| {
            let mut state = state.borrow_mut();
            let entry = state.entry(format!("{input_id}:drag")).or_default();
            let hit = self.rect_to_global(Rect::new(-0.025, cy - 0.045, len + 0.05, 0.09));
            self.ensure_touches();
            if entry.is_none() {
                if let Some(touch) = self
                    ...
                    .unwrap()
                    .iter()
                    .find(|it| it.phase == TouchPhase::Started && hit.contains(it.position))
                {
                    *entry = Some(touch.id);
                }
            }
            if let Some(id) = *entry {
                ...
                // Empty event frames retain ownership. A captured finger cannot click other controls.
                self.touches.as_mut().unwrap().retain(|it| it.id != id);
            }
            ...
            Rect::new(-0.025, -0.012, x - gap + 0.025, cy + 0.045 + 0.012)
        })
    }

    /// Clear practice captures when the scene changes, the app loses focus, or the pause UI closes.
