...
impl<T: Tweenable> Anim<T> {
    ...
    /// Read without moving the playback cursor or cloning keyframes.
    pub fn value_at(&self, time: f64) -> Option<T> {
        ...
        let value = if let Some(next) = self.keyframes.get(i + 1) {
            let t = (time - first.time) / (next.time - first.time);
            T::tween(&first.value, &next.value, first.tween.y(t as f32))
        } else {
            first.value.clone()
        };
        Some(if let Some(addition) = self.next.as_ref().and_then(|next| next.value_at(time)) {
            T::add(&value, &addition)
        } else {
            value
        })
    }
