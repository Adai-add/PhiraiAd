...
impl JudgeLineCache {
    ...
    /// Rebuild rendering groups after a kind change, keeping original-note data aligned.
    pub(crate) fn with_companion<T>(notes: &mut Vec<Note>, companion: &mut Vec<T>) -> Self {
        assert_eq!(notes.len(), companion.len());
        let mut paired: Vec<_> = std::mem::take(notes).into_iter().zip(std::mem::take(companion)).collect();
        paired.sort_by_key(|(it, _)| {
            (it.plain(), !it.above, it.speed.not_nan(), ((it.height + it.object.translation.1.now() as f64) * it.speed).not_nan())
        });
        (*notes, *companion) = paired.into_iter().unzip();
        let mut cache = Self {
            update_order: Vec::new(),
            not_plain_count: 0,
            above_indices: Vec::new(),
...
impl JudgeLine {
    ...
    pub fn render(&self, ui: &mut Ui, res: &mut Resource, lines: &[JudgeLine], bpm_list: &mut BpmList, settings: &ChartSettings, id: usize) {
        ...
        res.with_model(self.now_transform(res, lines), |res| {
            ...
            let note_flow_speed = res.note_flow_speed as f64;
            ...
            for index in &self.cache.above_indices {
                ...
                let limit = height_above as f64 / (speed * note_flow_speed);
            ...
            res.with_model(Matrix::identity().append_nonuniform_scaling(&Vector::new(1.0, -1.0)), |res| {
                ...
                for index in &self.cache.below_indices {
                    let speed = self.notes[*index].speed;
                    let limit = height_below as f64 / (speed * note_flow_speed);
                    for note in self.notes[*index..].iter() {
                        if speed != note.speed {
                            break;
                        }
